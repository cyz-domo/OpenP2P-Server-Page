...
- **多用户 / 跨浏览器会话隔离**：
  - 采用服务端 Session 会话机制（`openp2p_session` HttpOnly Cookie + `X-Session-Id` 双轨支持）；
  - **切换浏览器或无痕模式必须重新登录**，不同浏览器各持独立会话，彼此数据与状态完全隔离；
  - 退出登录仅注销当前浏览器会话，不影响其他同时在线的用户；
- **会话内多账户管理**：
  - 顶栏「👤 账户名 ▾」下拉可在当前会话内管理/切换多个 OpenP2P 账号；
  - 点选即切，设备、规则与组网数据即时刷新；
  - 会话默认 7 天有效，支持服务端持久化与密码过期自动重登。

---

## 📱 移动端深度适配 (Responsive Mobile Support)

- **自适应触控顶栏**：小屏幕下自动隐藏冗余文字，紧凑化图标与账户气泡，触控不误触；
- **弹性手势导航**：支持横向滑动的选项卡导航栏，顺滑流畅；
- **响应式 KPI 大盘**：双列卡片布局，信息密度与字体层阶针对手机屏幕调优；
- **全屏自适应弹窗与表单**：弹窗自适应 95vw 宽度，底部操作按钮支持大拇指易按排版，输入框优化防缩放；
- **下载中心与表格平滑横滚**：规则、设备与网络配置表原生横向滑动，右侧操作列常驻可见。
...
## 与官方控制台的功能对照（审计 2026-09-14，差距已全部补齐）

- ✅ 转发规则 **IP 白名单**（编辑弹窗字段，逗号分隔网段）
- ✅ **对端服务探测**（新建规则时可先探测目标端口可达性，MsgPushCheckRemoteService）
- ✅ **设备编辑**：改名 / 共享带宽 / 强制 IPv6 / 公网监听端口（官方同款 POST /device/<名>/edit）
- ✅ **忘记密码**：官方邮箱验证码流程（GET verificationcode + POST resetpwd）
- 唯一不适用：官方"我的"页的邮箱/手机绑定编辑（个人敏感操作，建议去官方控制台办理）
