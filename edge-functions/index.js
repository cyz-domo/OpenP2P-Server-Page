/**
 * OpenP2P 管理面板 — 腾讯云 EdgeOne Makers 边缘函数 (Edge Functions)
 * 纯 Serverless 边缘运行，无需任何服务器或 Python 环境
 */
...
export default {
  async fetch(request, env = {}, ctx) {
    ...
    // ---------- 反向代理 /pxp/* 转发到 OpenP2P 官方控制台 ----------
    if (pathname.startsWith("/pxp/")) {
      const session = await getSession(request, env);
      const upstream = session.upstream || env.UPSTREAM_URL || DEFAULT_UPSTREAM;
      const subPath = pathname.slice(4); // 去除 /pxp 前缀，保留前导 /
    ...
    // 如果未命中任何 API，交由 EdgeOne 静态资源路由托管
