  ...
  function jwtExp(token2) {
    const p = parseJwt(token2);
    return Number(p.exp || 0);
  }
  function jwtUser(token2) {
    const p = parseJwt(token2);
    return p.user || "";
  }
  function generateCaptchaSvg(code, theme = "dark") {
    ...
    const svgXml = `<svg xmlns="http://www.w3.org/2000/svg" width="120" height="44" viewBox="0 0 120 44"><rect width="120" height="44" fill="${bg}" rx="6"/><line x1="10" y1="12" x2="110" y2="34" stroke="${noise}" stroke-width="1"/><line x1="10" y1="36" x2="110" y2="10" stroke="${noise}" stroke-width="1"/>${chars}</svg>`;
  ...
  async function onRequest(context) {
    ...
    if (pathname === "/api/state" && method === "GET") {
      ...
      const user2 = session.activeUser || "";
      const acc = pool.find((a) => a.user === user2);
      const token2 = acc ? acc.token : "";
      return jsonRsp({
        upstream,
        user: user2,
        hasToken: Boolean(token2),
        tokenExp: token2 ? jwtExp(token2) : 0,
        accounts: pool.map((a) => ({
          user: a.user,
